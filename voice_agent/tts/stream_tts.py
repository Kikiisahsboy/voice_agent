...
新增 `synthesize_to_bytes(text)`：异步合成文本到 MP3 字节，供 web 流式推送。
"""
...
class StreamTTS:
    ...
    async def _synthesize_bytes_async(self, text: str) -> bytes:
        """异步合成文本到 MP3 字节（不本地播放）。"""
    ...
    def synthesize_to_bytes(self, text: str) -> bytes:
        """同步包装：合成文本到 MP3 字节，用于推送到浏览器。"""
        if not text or not text.strip():
            return b""
        try:
            return self._submit_coro(self._synthesize_bytes_async(text))
        except Exception as e:
            logger.warning("TTS 合成失败: %s", e)
            return b""

    def synthesize_to_base64(self, text: str) -> str:
        mp3 = self.synthesize_to_bytes(text)
        if not mp3:
            return ""
